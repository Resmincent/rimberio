    ...
    <div class="row mt-2">
        <div class="col-lg-8 mx-auto mt-2">
            <div class="card-body border-1" style="background-color: #FFFFFF; border-radius: 15px">
                ...
                <p class="text-gray-500" style="margin-left: 10px">Edit details for the user.</p>
                <div id="user-container">
                    <div id="user" class="m-2">
                        <label for="name" class="required form-label" style="color: #62717D">User Name</label>
                        <div class="input-group mb-5">
                            <input type="text" class="border-hover-success form-control @error('name') is-invalid @enderror" name="name" id="name" value="{{ old('name', $user->name) }}" placeholder="Type Name Here" />
                            @error('name')
                        ...
                        <div class="input-group mb-5">
                            <input type="email" class="border-hover-success form-control @error('email') is-invalid @enderror" name="email" id="email" value="{{ old('email', $user->email) }}" placeholder="Type Email Here" />
                            @error('email')
                            <small class="invalid-feedback" role="alert">{{ $message }}</small>
                            @enderror
                        </div>

                        <label for="password" class="form-label" style="color: #62717D">Password (Optional)</label>
                        <div class="input-group mb-5">
                            <input type="password" class="border-hover-success form-control @error('password') is-invalid @enderror" name="password" id="password" placeholder="Kosongkan untuk password yang sama" />
                            @error('password')
                            <small class="invalid-feedback" role="alert">{{ $message }}</small>
                            @enderror
                        </div>
